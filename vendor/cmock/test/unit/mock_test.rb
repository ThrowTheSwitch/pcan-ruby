...
class MockTest < Test::Unit::TestCase
	...
	def test_expect
		...
		mock.expect.absorb_something(:location, 'garbage')
	...
	def test_expect_using_arguments_for_method_and_arguments
		mock = Mock.new('order')
		mock.expect(:absorb_something, :location, 'garbage')
		mock.absorb_something(:location, 'garbage')
		mock._verify
	...
	def test_expect_using_arguments_for_method_and_arguments_with_block
		mock = Mock.new('order')
		mock.expect(:absorb_something, :location, 'garbage') { |a,b,block|
	...
	def test_expect_using_string_method_name
		mock = Mock.new('order')
		mock.expect('absorb_something', :location, 'garbage')
		mock.absorb_something(:location, 'garbage')
		mock._verify
	...
	def test_expect_assignment
		mock = Mock.new('order')
		mock.expect.account_number = 1234
	...
	def test_expect_assigment_using_arguments_for_method_and_arguments
		mock = Mock.new('order')
		mock.expect(:account_number=, 1234)
		mock.account_number = 1234
		mock._verify
	...
	def test_expect_assigment_using_string_method_name
		mock = Mock.new('order')
		mock.expect('account_number=', 1234)
		mock.account_number = 1234
		mock._verify
  ...
  def test_expect_assignment_and_return_is_overruled_by_ruby_syntax
		...
		mock = Mock.new('order')
		mock.expect(:account_number=, 1234).returns "gold"
		got = mock.account_number = 1234
		mock._verify
		assert_equal 1234, got, "Expected rvalue"
  end

  def test_expect_assignment_and_raise
		mock = Mock.new('order')
		mock.expect(:account_number=, 1234).raises StandardError.new("kaboom")
	...
	def test_expect_multiple
		mock = Mock.new('order')
		control = mock._control

		assert control.happy?

		mock.expect.one_thing :hi, { :goose => 'neck' }
		mock.expect.another 5,6,7
		assert !control.happy?
  ...
  def test_wrong_call
	...
	def test_expect_and_return
		mock = Mock.new('order')
		mock.expect.delivery_date.returns Date.today
		assert_equal Date.today, mock.delivery_date
		mock._verify
	end

	def test_expect_and_return_with_arguments
		mock = Mock.new('order')
		mock.expect.delivery_date(:arf,14).returns(Date.today)
		assert_equal Date.today, mock.delivery_date(:arf,14)
		mock._verify
	end

	def test_expect_and_raise
		mock = Mock.new('order')
		mock.expect.delivery_date.raises StandardError.new("bloof")
	...
	def test_expect_a_runtime_block
		mock = Mock.new('order')
		got_val = nil

		mock.expect.when(:something) { |e,block|
			got_val = block.call
		}
